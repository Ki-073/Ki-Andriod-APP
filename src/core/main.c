...
static void write_root_script(void) {
  ...
  int n = snprintf(
      ...
      "if [ ! -x \"$KSUD\" ]; then\n"
      "  KSUD=$(find /data/app -path '*/com.sukisu.ultra*/lib/arm64/libksud.so' 2>/dev/null | head -1)\n"
      "fi\n"
      "if [ ! -x \"$KSUD\" ]; then\n"
      ...
      "fi\n"
      "if [ ! -x \"$KSUD\" ]; then KSUD=/data/local/tmp/ksud; fi\n"
      "if [ ! -x \"$KSUD\" ]; then KSUD=/data/adb/ksud; fi\n"
