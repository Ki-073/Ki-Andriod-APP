...
fn run(cli: &Cli) -> Result<i32> {
    ...
    if cli.image.to_string_lossy().starts_with("http://")
        || cli.image.to_string_lossy().starts_with("https://")
    {
        return Err(ExtractError::new(
            "this build has payload/OTA-URL support disabled; pass a local boot.img or raw Image",
        ));
    }
